...
from gradmarket.sources import ashby

FIXTURES = Path(__file__).resolve().parent / "fixtures"
JOBS_200 = json.loads((FIXTURES / "ashby_postings.json").read_text())
...
def no_sleep(monkeypatch):
    calls = []
    monkeypatch.setattr(ashby.time, "sleep", lambda s: calls.append(s))
    return calls
...
def test_fetch_429_retries_then_succeeds(monkeypatch):
    ...
    result = ashby.fetch("ratelimited")
...
def test_fetch_5xx_exhausts_retries(monkeypatch):
    ...
    result = ashby.fetch("down")

    assert result.status_code == 503
    assert result.payload is None
    assert len(calls) == ashby.MAX_RETRIES + 1
    assert sleeps == ashby.BACKOFF_SECONDS
