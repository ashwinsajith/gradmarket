...
def test_rule1_campus_recruiter_beats_rule4_campus():
    # Contains both a rule-1 signal (recruiter) and what would be a rule-4
    # early signal (campus) — rule 1 is checked first and wins. Rule 1 is
    # NOT part of the rule-2 conflict check.
    assert classify_seniority("Campus Recruiter", "") == "experienced"


def test_rule1_head_of_early_career_recruiting_beats_rule4_early_career():
    assert classify_seniority("Head of Early Career Recruiting", "") == "experienced"


# --- rule 2: title has BOTH an experienced marker and an early keyword -> early ---


def test_rule2_junior_software_engineering_manager():
    # "Manager" (rule 3) and "Junior" (rule 4) both present — early wins.
    assert classify_seniority("Junior Software Engineering Manager", "") == "early"


def test_rule2_conflict_beats_plain_experienced_marker_precedence():
    # Sanity check on ordering: without the early keyword, "Manager" alone
    # is still experienced (rule 3) — the conflict only fires when both are
    # present.
    assert classify_seniority("Software Engineering Manager", "") == "experienced"


# --- four real holdout-evaluation failures, all from the "Specialist" bug ---
# "Specialist" used to be an experienced marker, so it hit (old) rule 2
# before junior/graduate ever reached (old) rule 3. Now that "Specialist" is
# not a marker at all, none of these four have any experienced-marker match,
# so they resolve on the early-title check alone — no conflict-rule needed
# for these specifically, but they're the reason both changes exist.


def test_holdout_failure_junior_platform_specialist():
    assert classify_seniority("Junior Platform Specialist", "") == "early"


def test_holdout_failure_trading_infrastructure_specialist_graduate_programme():
    assert classify_seniority("Trading Infrastructure Specialist – Graduate Programme", "") == "early"


def test_holdout_failure_junior_early_growth_specialist():
    assert classify_seniority("Junior Early Growth Specialist CR German", "") == "early"


def test_holdout_failure_junior_data_acquisition_specialist():
    assert classify_seniority("Junior Data Acquisition Specialist", "") == "early"


# --- rule 3: senior-type titles (no early keyword present) ---


def test_rule3_senior():
    assert classify_seniority("Senior Software Engineer", "") == "experienced"


def test_rule3_staff():
    assert classify_seniority("Staff Engineer", "") == "experienced"


def test_rule3_principal():
...
def test_rule7_no_evidence_defaults_to_experienced():
    assert classify_seniority("Software Engineer", "Join our team and build great things.") == "experienced"


def test_rule7_title_only_no_signal_defaults_to_experienced():
