...
def test_external_id_from_bullet_fields_first_element():
    ...
    postings = parse_workday.extract([item], {}, TOKEN)
...
def test_content_hash_changes_once_detail_arrives():
    ...
    without_detail = parse_workday.extract([item], {}, TOKEN)[0]
    with_detail = parse_workday.extract(
        [item],
        {"JR0001": detail(description="<p>Join us.</p>", location="Edinburgh", country="United Kingdom")},
        TOKEN,
    )[0]
...
def test_detail_present_fills_description_and_location():
    ...
    posting = parse_workday.extract([item], details, TOKEN)[0]
...
def test_multiple_postings_mixed_detail_availability():
    ...
    postings = parse_workday.extract(items, details, TOKEN)
